...
const ExploreContainer: React.FC = () => {
  const { state, selectPeriod, showContent, setCurrentContent, setLoading } = useAppState();
  const historicalDataService = HistoricalDataService.getInstance();

  const handleStartLearning = async (period: HistoricalPeriod) => {
    setLoading(true);
    try {
      const periodWithContent = await historicalDataService.getPeriodWithContent(period.id);
  ...
  return (
    <Routes>
      ...
      <Route path="/" element={<LearningInterface />}>
        ...
        <Route path="home" element={<LearningInterface />} />
